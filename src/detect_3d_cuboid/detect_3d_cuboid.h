...
public:
    ...
    // object detector needs image, camera pose, and 2D bounding boxes(n*5, each row: xywh+prob+ID)
    // BB xy are the top-left corner
    // ID is the id assigned to the trajectory after object tracking
    void detect_cuboid(const cv::Mat &rgb_img, const Eigen::Matrix4d &transToWolrd,
