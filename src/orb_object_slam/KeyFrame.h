...
public:
    ...
    std::vector<KeyFrame *> GetBestCovisibilityKeyFrames(
        const int &N); // select the topN similar images, that have most point matches.
    ...
    std::vector<MapPoint *>
    GetMapPointMatches(); // same length as keypoints. if keypoint not matched, nullptr
...
protected:
    ...
    std::vector<MapPoint *> mvpMapPoints; // has nothing to do with Frame. mvpMapPoints after
                                          // copying! private variable!!!! only add by AddMapPoint()
