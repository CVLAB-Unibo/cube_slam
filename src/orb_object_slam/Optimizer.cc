            ...
            if (pMP)
            {
                ...
                if (pFrame->mvuRight[i] < 0)
                {
                    ...
                    g2o::EdgeSE3ProjectXYZOnlyPose *e =
                        new g2o::EdgeSE3ProjectXYZOnlyPose(); // create many unary edges
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap)
{
    ...
    const vector<KeyFrame *> vNeighKFs =
        pKF->GetVectorCovisibleKeyFrames(); // directly get local keyframes.
         ...
         lit != lend; lit++)
    {
             ...
             vit++)
        {
            ...
            if (pMP)
                if (!pMP->isBad())
                    if (pMP->mnBALocalForKF !=
                        pKF->mnId) // mnBALocalForKF  mnBAFixedForKF are marker
                    {
                        if (whether_dynamic_object && pMP->is_dynamic)
                            continue;
         ...
         lit != lend; lit++)
    {
             ...
             mit != mend; mit++)
        {
            ...
            if (pKFi->mnBALocalForKF != pKF->mnId &&
                pKFi->mnBAFixedForKF !=
                    pKF->mnId) // not been added to lLocalKeyFrames, and lFixedCameras!
            {
         ...
         lit != lend; lit++)
    {
        ...
        if (pMP->Observations() == 1) // by me, skip observation one point, which is just
                                      // initialized by my depth! won't affect previous mono.
            continue;
            ...
            if (!pKFi->isBad())
            {
                ...
                if (pKFi->mvuRight[mit->second] < 0)
                {
                    ...
                    g2o::EdgeSE3ProjectXYZ *e =
                        new g2o::EdgeSE3ProjectXYZ(); // camera point edge.  there is no
                                                      // camera-camera odometry edge.

                    e->setVertex(
                        0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                        optimizer.vertex(pKFi->mnId))); // get vertex based on Id.
                    e->setMeasurement(obs);
         ...
         lit != lend; lit++)
    {
        ...
        g2o::VertexSE3Expmap *vSE3 =
            static_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(pKFrame->mnId));
        g2o::SE3Quat SE3quat = vSE3->estimate();
...
void Optimizer::LocalBACameraPointObjects(KeyFrame *pKF, bool *pbStopFlag, Map *pMap,
                                          bool fixCamera, bool fixPoint)
{
    // Local KeyFrames to optimize: First Breath Search from Current Keyframe
    ...
    for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
         lit != lend; lit++)
    {
        vector<MapPoint *> vpMPs = (*lit)->GetMapPointMatches();
        for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend;
         ...
         lit != lend; lit++)
    {
        ...
        for (vector<MapObject *>::iterator vit = vpMOs.begin(), vend = vpMOs.end(); vit != vend;
             vit++)
        {
            MapObject *pMO = *vit;
            if (pMO)
                if (!pMO->isBad())
                    if (pMO->mnBALocalForKF !=
                        pKF->mnId) // mnBALocalForKF  mnBAFixedForKF are marker
                    {
    ...
    for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
         lit != lend; lit++)
    {
        map<KeyFrame *, size_t> observations = (*lit)->GetObservations();
        for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
    ...
    // 改变的地方
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
                                       lend = lLocalMapObjects.end();
         lit != lend; lit++)
    {
    ...
    std::cout << "LocalBAPointObjects num of local points / objects / frames:  "
              << lLocalMapPoints.size() << "  " << lLocalMapObjects.size() << "  "
              << lLocalKeyFrames.size() << std::endl;
...
#define ObjectFixScale // use when scale is provided and fixed. such as KITTI
    ...
    // Set Fixed KeyFrame vertices
    for (vector<KeyFrame *>::iterator lit = lFixedCameras.begin(), lend = lFixedCameras.end();
         lit != lend; lit++)
    {
        KeyFrame *pKFi = *lit;
        g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
         ...
         lit != lend; lit++)
    {
        ...
        if (scene_unique_id == kitti)
        {
            ...
            {
                float cam_height = pKF->GetCameraCenter().at<float>(
                    1); // in init frame, y downward for KITTI
                        // 	cube_pose.setTranslation(Vector3d(cube_pose.translation()(0),1.7-0.7631,cube_pose.translation()(2)));
                cube_pose.setTranslation(Vector3d(cube_pose.translation()(0), cam_height + 1.0,
                                                  cube_pose.translation()(2)));
            }
            else
            {
                float cam_height = pKF->GetCameraCenter().at<float>(2);
                cube_pose.setTranslation(Vector3d(cube_pose.translation()(0),
                                                  cube_pose.translation()(1), cam_height - 1.0));
            }
            if (vObject->fixedscale(0) >
                0) // even later didn't actually optimize, change the scale. just for visualization.
         ...
         lit != lend; lit++)
    {
        ...
        if (pMP->Observations() == 1) // HACK by me, skip observation one point, which is
                                      // initialized by my depth! won't affect previous mono.
            continue;
    ...
    if (1)
    {
        // get object point association.can we do this when validate cuboids?? then don't need to do
        // it every time?
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++)
        {
            ...
            point_object_threshold = std::max(int(largest_point_obs_num * 0.4),
                                              2); // whether use adaptive threshold or fixed.
                ...
                if (UniquePoints[j])
                    if (!UniquePoints[j]->isBad())
                        if (UniquePoints[j]->MapObjObservations[pMO] > point_object_threshold)
                        {
                            ...
                            all_object_ba_points[i].push_back(
                                Converter::toVector3d(UniquePoints[j]->GetWorldPos()));
                        }
        }
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++)
        {
            ...
            std::vector<Eigen::Vector3d>
                good_points; // for car, if points are 4 meters away from center, usually outlier.
            for (size_t j = 0; j < all_object_ba_points[i].size(); j++)
            {
            ...
            if ((all_object_ba_points[i].size() > 5) &&
                1) // whether want to initialize object position to be center of points
            {
                g2o_object_vertex *vObject =
                    static_cast<g2o_object_vertex *>(optimizer.vertex(pMObj->mnId + maxKFid + 1));
        ...
        for (size_t i = 0; i < lLocalMapObjects.size();
             i++) // no need to optimize all objects...., use local KF's map objects?
        {
            MapObject *pMO = lLocalMapObjects[i];
            ...
            {
#ifdef ObjectFixScale
                g2o::EdgePointCuboidOnlyObjectFixScale *e =
                    new g2o::EdgePointCuboidOnlyObjectFixScale();
#else
                ...
                if (e->object_points.size() > 10)
                {
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                        optimizer.vertex(pMO->mnId + maxKFid + 1)));
                    Eigen::Matrix3d info;
                    info.setIdentity();
                    e->setInformation(info);
            ...
            if (0) // each object is connect to one point. optimize both point and object
            {
                if (pMO->used_points_in_BA_filtered.size() > 10)
                    for (size_t j = 0; j < pMO->used_points_in_BA_filtered.size(); j++)
                    {
                        ...
                        g2o::OptimizableGraph::Vertex *pointvertex =
                            dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                optimizer.vertex(pMO->used_points_in_BA_filtered[j]->mnId +
                                                 maxKFid + maxObjectid + 2));
            ...
            // 	  ROS_ERROR_STREAM("BA filter size/e objec point size
            // "<<pMO->used_points_in_BA_filtered.size()<<"   "<<all_object_ba_points[i].size());
        }
    }
    ...
    if (1)
    {
        ...
        inv_sigma.setOnes();
        inv_sigma = inv_sigma * camera_object_BA_weight; // point sigma<1, object error is usually
                                                         // large, no need to set large sigma...
        ...
        if (whether_want_camera_obj)
                 ...
                 lit != lend; lit++)
            {
                ...
                for (unordered_map<KeyFrame *, size_t>::const_iterator mit = observations.begin(),
                                                                       mend = observations.end();
                     mit != mend; mit++)
                {
                    KeyFrame *pKFi = mit->first;
                    ...
                    if (!pKFi->isBad())
                    {
                        ...
                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                            optimizer.vertex(pKFi->mnId))); // camera
                        e->setVertex(1,
                                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                         optimizer.vertex(pMObject->mnId + maxKFid + 1))); // object
                        ...
                        if ((bbox_2d.x > object_boundary_margin) &&
                            (bbox_2d.y > object_boundary_margin) &&
                            (bbox_2d.x + bbox_2d.width <
                             pMap->img_width - object_boundary_margin) &&
                            (bbox_2d.y + bbox_2d.height <
                             pMap->img_height - object_boundary_margin))
                        {
                            e->Kalib = pMap->Kalib;
                            e->setInformation(camera_object_sigma * pMObject->meas_quality *
                                              pMObject->meas_quality);
                ...
                if (obj_obs_num ==
                    1) // if an object is only connected to one camera, don't optimize it! no need
                {
                    obj_edge->setLevel(1);
                }
            }
        ...
        if (scene_unique_id == kitti) // balance left-right cars.
        {
            if (total_left > 2 * (total_right + total_middle))
            {
                for (size_t i = 0; i < vpEdgesCameraObjectLeft.size(); i++)
                    vpEdgesCameraObjectLeft[i]->setInformation(
                        vpEdgesCameraObjectLeft[i]->information() / 2.0);
            }
            if (total_right > 2 * (total_left + total_middle))
            {
                for (size_t i = 0; i < vpEdgesCameraObjectRight.size(); i++)
                    vpEdgesCameraObjectRight[i]->setInformation(
                        vpEdgesCameraObjectRight[i]->information() / 2.0);
            }
        }
    }

    std::cout << "BA edges  point-cam  object-cam  "
              << vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size() << "  "
              << vpEdgesCameraObject.size() << std::endl;
         ...
         lit != lend; lit++)
    {
        ...
        g2o::VertexSBAPointXYZ *vPoint = static_cast<g2o::VertexSBAPointXYZ *>(
            optimizer.vertex(pMP->mnId + maxKFid + maxObjectid + 2));
        pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate()));
        pMP->UpdateNormalAndDepth();
    }
         ...
         lit != lend; lit++)
    {
        ...
        g2o_object_vertex *vObject =
            static_cast<g2o_object_vertex *>(optimizer.vertex(pMObject->mnId + maxKFid + 1));
        pMObject->SetWorldPos(vObject->estimate());
    }
}

// similar to localBA, add objects
void Optimizer::LocalBACameraPointObjectsDynamic(KeyFrame *pKF, bool *pbStopFlag, Map *pMap,
                                                 bool fixCamera, bool fixPoint)
{
    // Local KeyFrames to optimize: First Breath Search from Current Keyframe
    vector<KeyFrame *> lLocalKeyFrames; // local KFs which share map points with current frame.
    ...
    {
        for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(),
    ...
    cout << "delete old single map points   " << deleteoldsinglepts << "   " << deleteoldsinglepts2
         << endl;
         ...
         lit != lend; lit++)
    {
             ...
             mit != mend; mit++)
        {
            ...
            if ((pKFi->mTimeStamp - pKF->mTimeStamp) > 8.0 && pKFi->mnBALocalForKF != pKF->mnId &&
                pKFi->mnBAFixedForKF !=
                    pKF->mnId) // not been added to lLocalKeyFrames, and lFixedCameras!
            {
                pKFi->mnBAFixedForKF = pKF->mnId;
...
#define ObjectFixScale
    ...
    linearSolver =
        new g2o::LinearSolverDense<g2o::BlockSolverX::PoseMatrixType>(); // LinearSolverEigen
                                                                         // LinearSolverDense
    g2o::BlockSolverX *solver_ptr = new g2o::BlockSolverX(linearSolver);
#else
    g2o::BlockSolverX::LinearSolverType *linearSolver; // BlockSolverX instead of BlockSolver63
         ...
         lit != lend; lit++)
    {
             ...
             mit != mend; mit++)
        {
            ...
            if (pKFi->mnBALocalForKF != pKF->mnId &&
                pKFi->mnBAFixedForKF != pKF->mnId) // not used in BA
                continue;
...
#ifdef ObjectFixScale
            if (scene_unique_id == kitti)
                vObject->fixedscale = Eigen::Vector3d(
                    1.9420, 0.8143, 0.7631); // for kitti object, scale may don't need to set...
            else
                ROS_ERROR_STREAM("Please see cuboid scale!!!, otherwise use VertexCuboid()");
            ...
            if (scene_unique_id == kitti)
            {
                if (!build_worldframe_on_ground)
                {
                    float cam_height = pKFi->GetCameraCenter().at<float>(
                        1); // in init frame, y downward for KITTI
                            // 		  cube_pose.setTranslation(Vector3d(cube_pose.translation()(0),1.7-0.7631,cube_pose.translation()(2)));
                    cube_pose.setTranslation(Vector3d(cube_pose.translation()(0), cam_height + 1.0,
                                                      cube_pose.translation()(2)));
                ...
                {
                    float cam_height =
                        pKFi->GetCameraCenter().at<float>(2); // in init frame, y downward for KITTI
                }
                if (vObject->fixedscale(0) > 0) // even later didn't actually optimize, change the
                                                // scale. just for visualization.
                    cube_pose.setScale(vObject->fixedscale);
            }
#endif
         ...
         lit != lend; lit++)
    {
        ...
        if (fixPoint)
            ...
            vPoint->setMarginalized(true); // must be true, otherwise run error. Not sure why. from
                                           // g2o paper, marginalize is just one way to solve...
        optimizer.addVertex(vPoint);
    ...
    if (ba_dyna_pt_obj_cam)
    {
             ...
             lit != lend; lit++)
        {
            ...
            if (pMP->Observations() < 4) // 3 should have more frame observations... otherwise
                                         // object depth inited map point is not accurate.
                continue;
            ...
            g2o::VertexSBAPointXYZ *vPoint = new g2o::VertexSBAPointXYZ();
            vPoint->setEstimate(
                Converter::toVector3d(pMP->PosToObj)); // dynamic point use point-object pose
            ...
            if (fixPoint)
                ...
                vPoint->setMarginalized(
                    true); // must be true, otherwise run error. Not sure why. from g2o paper,
                           // marginalize is just one way to solve...
            optimizer.addVertex(vPoint);

            g2o::UnaryLocalPoint *e2 = new g2o::UnaryLocalPoint();
            e2->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                 optimizer.vertex(id))); // local point
            Eigen::Matrix3d info;
            info.setIdentity();
                 ...
                 mit != mend; mit++)
            {
                ...
                if (!pKFi->isBad())
                {
                    ...
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                        belongedobj->bundle_vertex_ids[pKFi]))); // object
                    e->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                        optimizer.vertex(id))); // point
    ...
    if (ba_dyna_obj_velo)
    {
        ...
        inv_sigma.setOnes();
        inv_sigma(2) =
            inv_sigma(2) * 5.0; // 0.1 angle error is usually much smaller compared to position.
             ...
             lit != lend; lit++)
        {
            ...
            if (pMObject->bundle_vertex_ids.size() <
                4) // 5 if only with few frames, don't optimize velocity. not accurate. meaningful
                   // for at least 3 objs
                continue;
            ...
            for (size_t i = 0; i < objkfs.size(); i++)
            {
                ...
                {
                    if ((pKF->mTimeStamp - obskf->mTimeStamp) >
                        5.0) // HACK just use recent 10s frames.
                    {
                        continue;
                    }
                    ...
                    else // add velocity edge
                    {
                        ...
                        e->setVertex(0,
                                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                         pMObject->bundle_vertex_ids[prevgoodobsframe])));
                        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                            optimizer.vertex(pMObject->bundle_vertex_ids[obskf])));
                        e->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(vVelocity));
                        e->delta_t = obskf->mTimeStamp - prevgoodobsframe->mTimeStamp;
                        // 		  cout<<"add obj id for frames:   "<<pMObject->mnId<<"
                        // "<<prevgoodobsframe->mnId<<"   "<<obskf->mnId<<"    delta "<<e->delta_t
                        // <<endl; 		  cout<<"obskf->mTimeStamp   "<<obskf->mTimeStamp<<"
                        // "<<prevgoodobsframe->mTimeStamp<<endl;
            ...
            if (pMObject->velocityPlanar(0) == 0 && pMObject->velocityPlanar(1) == 0 &&
                firstgoodobsframe && lastgoodobsframe)
            {
                ...
                double linear_esti =
                    (last_pose.pose.translation() - init_pose.pose.translation()).norm() /
                    time_range;
                ...
                // 	     ROS_ERROR_STREAM("pMObject->velocityPlanar  estimate
                // "<<pMObject->velocityPlanar.transpose());
                vVelocity->setEstimate(pMObject->velocityPlanar);
            }
            // 	  ROS_ERROR_STREAM("object observed frames   "<<pMObject->mnId<<"
            // "<<valid_observed_frames);
        }
    }
                    ...
                    if (!pKFi->isBad())
                    {
                        ...
                        e->setVertex(
                            1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                   optimizer.vertex(pMObject->bundle_vertex_ids[pKFi]))); // object
    ...
    std::cout << "BA edges  point-cam: " << vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size()
              << "   object-cam: " << vpEdgesCameraObject.size()
              << "   pt-obj-cam: " << vpEdgesCameraPointObject.size()
              << "   pt-obj: " << vpEdgesPointObject.size()
              << "   obj-vel: " << allmotionedges.size() << std::endl;
         ...
         lit != lend; lit++)
    {
        ...
        if (pMP->is_dynamic)
            continue;
        g2o::VertexSBAPointXYZ *vPoint = static_cast<g2o::VertexSBAPointXYZ *>(
            optimizer.vertex(pMP->mnId + maxIdTillObject + 1));
        pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate()));
        pMP->UpdateNormalAndDepth();
    }
    // fixed frames.
    ...
    {
        MapObject *pMObject = *lit;
        // 	ROS_ERROR_STREAM("Object ID   "<<pMObject->mnId<<"   obsNum
             ...
             mit != mend; mit++)
        {
            ...
            {
                valid_observed_frames++;
                g2o_object_vertex *vObject = dynamic_cast<g2o_object_vertex *>(
                    optimizer.vertex(pMObject->bundle_vertex_ids[pKFi]));
                pMObject->allDynamicPoses[pKFi] = make_pair(vObject->estimate(), true);
                if (latest_observed_goodframe == nullptr ||
                    (pKFi->mnId > latest_observed_goodframe->mnId))
        ...
        g2o::VelocityPlanarVelocity *vVelocity =
            dynamic_cast<g2o::VelocityPlanarVelocity *>(optimizer.vertex(velocity_id));
        if (vVelocity)
        {
    ...
    if (ba_dyna_pt_obj_cam)
    {
             ...
             lit != lend; lit++)
        {
            ...
            g2o::VertexSBAPointXYZ *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(
                optimizer.vertex(pMP->mnId + maxIdTillObject + 1));
            if (!vPoint)
                continue;
            ...
            pMP->mWorldPos_latestKF =
                Converter::toCvMat(belongedobj->pose_Twc_latestKF.pose * (vPoint->estimate()));
