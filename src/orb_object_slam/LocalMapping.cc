...
void LocalMapping::bundle_adjustment_caller()
{
    if (whether_detect_object && bundle_object_opti)
    {
        if (whether_dynamic_object)
            Optimizer::LocalBACameraPointObjectsDynamic(mpCurrentKeyFrame, &mbAbortBA, mpMap, false,
                                                        false);
        else
            Optimizer::LocalBACameraPointObjects(mpCurrentKeyFrame, &mbAbortBA, mpMap, false,
                                                 false);
    }
    else
    {
...
{
    //     Tracking will see that Local Mapping is busy   if BA processing KF now, don't accept new
    //     KF
    ...
    if (CheckNewKeyFrames())
    {
        ...
        if (!CheckNewKeyFrames() && !stopRequested()) // after all keyframes being inserted
        {
            ...
            {
                // don't immediately BA if first frame is depth intialized, only a few frames,
                // baseline is small, will damage the map.
                if ((mbMonocular) &&
                    (mono_firstframe_truth_depth_init || mono_firstframe_Obj_depth_init) &&
            ...
            // Check redundant local Keyframes, could delete frame is many points in this frame are
            // observed before
            KeyFrameCulling();
        }
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    for (size_t i = 0; i < vpMapPointMatches.size();
         i++) // add frame observation for associated map points
    {
        MapPoint *pMP = vpMapPointMatches[i];
        // in mono/stereo/RGBD depth initialization, or stereo/RGBG create new KF,   new created map
        // points already added to observation in tracking, there is also matched points to previous
        // frame, not yet add to observation
        ...
        if (pMP)
        {
            ...
            {
                if (!pMP->IsInKeyFrame(
                        mpCurrentKeyFrame)) // associated   not yet add to observation
                {
                    newaddobservation++;
    ...
    {
        const vector<MapPoint *> vpHarrisMapPointMatches =
            mpCurrentKeyFrame->GetHarrisMapPointMatches();
    ...
    std::cout << "Mapping: recent buffer pts:  " << mlpRecentAddedMapPoints.size()
              << "   Frame created pts:  " << newaddRecent
...
void LocalMapping::MapPointCulling()
{
    ...
    {
        nThObs = 2;
        // my new added point from Monocular depth initilalization only has one observation, should
        // not delete
...
template <class BidiIter> // Fisher-Yates shuffle    only the first num_random of the output vector
                          // is shuffled
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for (size_t i = 0; i < vpNeighKFs.size(); i++)
    {
        ...
        for (int ikp = 0; ikp < nmatches; ikp++)
        {
            ...
            {
                if (mpCurrentKeyFrame->KeysStatic.size() > 0 &&
                    !mpCurrentKeyFrame->KeysStatic[idx1])
                    continue;
            ...
            const float cosParallaxRays =
                ray1.dot(ray2) /
                (cv::norm(ray1) * cv::norm(ray2)); // parallax in keyframe 1 and keyframe 2
    ...
    if (mono_allframe_Obj_depth_init && !whether_dynamic_object)
    {
        ...
        if (1)
        {
            ...
            {
                std::vector<int> has_object_depth_pixel_inds; // points with no depth yet but with
                                                              // matching object
                ...
                max_initialize_pts = std::min(int(total_feature_pts * 0.30) - int(raw_depth_pts),
                ...
                {
                    srand(time(NULL));
                    random_unique(has_object_depth_pixel_inds.begin(),
