                             ...
                             cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    if (RH > 0.40)
    {
        ...
        return ReconstructH(vbMatchesInliersH, H, mK, R21, t21, vP3D, vbTriangulated, min_parallax,
                            50);
    }
    else // if(pF_HF>0.6)
    {
        std::cout << "come to compute fundamental" << std::endl;
        return ReconstructF(vbMatchesInliersF, F, mK, R21, t21, vP3D, vbTriangulated, min_parallax,
                            50);
    }
