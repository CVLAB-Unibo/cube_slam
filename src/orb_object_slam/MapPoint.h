...
public:
    ...
    void EraseObservation(
        KeyFrame *pKF); // called in Keyframe when set bad, and in optimizer when removing outliers
    ...
    void Replace(MapPoint *pMP); // usually means replace close point, with similar decriptor.
                                 // called in matcher.Fuse(), in localmapping
    ...
    void ComputeDistinctiveDescriptors(); // select/update map point's decriptors based on frame's
                                          // keypoints' decriptors
