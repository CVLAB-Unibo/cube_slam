...
SE3Quat exptwist_norollpitch(const Vector6d &update)
{
    ...
    Matrix3d R;
    R << cos(omega(2)), -sin(omega(2)), 0, sin(omega(2)), cos(omega(2)), 0, 0, 0, 1;
...
void VertexCuboid::oplusImpl(const double *update_)
{
    ...
    else if (whether_fixrollpitch) // NOTE this only works for cuboid already has parallel to
                                   // ground. otherwise update_z will also change final RPY
    {
        ...
        newcube.pose =
            _estimate.pose *
            exptwist_norollpitch(update2.head<6>()); // NOTE object pose is from object to world!!!!
    }
    else
        newcube.pose = _estimate.pose * SE3Quat::exp(update.head<6>());

    if (whether_fixheight) // use previous height
        newcube.setTranslation(Vector3d(newcube.translation()(0), _estimate.translation()(1),
                                        newcube.translation()(2)));
...
{
    const VertexSE3Expmap *SE3Vertex =
        dynamic_cast<const VertexSE3Expmap *>(_vertices[0]); //  world to camera pose
    const VertexCuboidFixScale *cuboidVertex =
        dynamic_cast<const VertexCuboidFixScale *>(_vertices[1]); //  object pose to world

    SE3Quat cam_pose_Tcw = SE3Vertex->estimate();
    cuboid global_cube = cuboidVertex->estimate();

    Vector4d rect_project =
        global_cube.projectOntoImageBbox(cam_pose_Tcw, Kalib); // center, width, height
...
void EdgeSE3CuboidProj::computeError()
{
    const VertexSE3Expmap *SE3Vertex =
        dynamic_cast<const VertexSE3Expmap *>(_vertices[0]); //  world to camera pose
    const VertexCuboid *cuboidVertex =
        dynamic_cast<const VertexCuboid *>(_vertices[1]); //  object pose to world
...
void EdgeDynamicPointCuboidCamera::computeError()
{
    const VertexSE3Expmap *SE3Vertex =
        dynamic_cast<const VertexSE3Expmap *>(_vertices[0]); // world to camera pose
    const VertexCuboidFixScale *cuboidVertex =
        dynamic_cast<const VertexCuboidFixScale *>(_vertices[1]); // object to world pose
    const VertexSBAPointXYZ *pointVertex =
        dynamic_cast<const VertexSBAPointXYZ *>(_vertices[2]); // point to object pose

    Vector3d localpt =
        SE3Vertex->estimate() * (cuboidVertex->estimate().pose * pointVertex->estimate());

    Vector2d projected = Vector2d(Kalib(0, 2) + Kalib(0, 0) * localpt(0) / localpt(2),
                                  Kalib(1, 2) + Kalib(1, 1) * localpt(1) / localpt(2));
...
{
    const VertexCuboidFixScale *cuboidVertexfrom =
        dynamic_cast<const VertexCuboidFixScale *>(_vertices[0]); // object to world pose
    const VertexCuboidFixScale *cuboidVertexto =
        dynamic_cast<const VertexCuboidFixScale *>(_vertices[1]); // object to world pose
    const VelocityPlanarVelocity *velocityVertex =
        dynamic_cast<const VelocityPlanarVelocity *>(_vertices[2]); // object to world pose
    ...
    Vector3d trans_back_pred =
        posefrom.translation() +
        (velocity(0) * delta_t - vehicle_length * 0.5) * Vector3d(cos(yaw_from), sin(yaw_from), 0);
    double yaw_pred = yaw_from + tan(velocity(1)) * delta_t / vehicle_length * velocity(0);

    // as mentioned in paper: my object frame is at the center. the motion model applies to back
    // wheen center. have offset.
    Vector3d trans_pred =
        trans_back_pred + vehicle_length * 0.5 * Vector3d(cos(yaw_pred), sin(yaw_pred), 0);

    _error = Vector3d(poseto.translation()[0], poseto.translation()[1], yaw_to) -
             Vector3d(trans_pred(0), trans_pred(1), yaw_pred);
...
Vector3d cuboid::point_boundary_error(const Vector3d &world_point,
                                      const double max_outside_margin_ratio,
                                      double point_scale) const
{
    // transform the point to local object frame  TODO actually can compute gradient analytically...
    Vector3d local_pt =
        point_scale *
        (this->pose.inverse() * world_point)
            .cwiseAbs(); // change global point to local cuboid body frame.  make it positive.
    Vector3d error;
    ...
    for (int i = 0; i < 3; i++)
    {
        if (local_pt(i) < this->scale(i))
            ...
            error(i) = max_outside_margin_ratio *
                       this->scale(i); // if points two far, give a constant error, don't optimize.
    }
...
{
    const VertexCuboid *cuboidVertex =
        dynamic_cast<const VertexCuboid *>(_vertices[0]); // world to camera pose
    ...
    for (size_t i = 0; i < object_points.size();
         i++) // use abs  otherwise   pos neg will counteract by different pts.     maybe each edge
              // one pt?
        point_edge_error +=
            estimate_cube.point_boundary_error(object_points[i], max_outside_margin_ratio)
                .cwiseAbs();
    ...
    Vector3d prior_shape_error =
        estimate_cube.scale; // setZero?  or penalize large box! or set a range?
    if (prior_object_half_size(0) >
        0) // if prior shape is being set, such as KITTI, then give large weight for shape error
    {
        prior_weight = 50.0;
        prior_shape_error = ((estimate_cube.scale - prior_object_half_size).array() /
                             prior_object_half_size.array())
                                .cwiseAbs();
    }
...
{
    const VertexCuboidFixScale *cuboidVertex =
        dynamic_cast<const VertexCuboidFixScale *>(_vertices[0]); // world to camera pose
...
{
    const VertexSBAPointXYZ *pointVertex =
        dynamic_cast<const VertexSBAPointXYZ *>(_vertices[0]); // point position
    const VertexCuboid *cuboidVertex =
        dynamic_cast<const VertexCuboid *>(_vertices[1]); //  object pose to world
    const g2o::cuboid estimate_cube = cuboidVertex->estimate();

    Vector3d point_edge_error =
        estimate_cube.point_boundary_error(pointVertex->estimate(), max_outside_margin_ratio)
            .cwiseAbs(); // abs to add shape error
    point_edge_error = point_edge_error.array() / estimate_cube.scale.array();
...
void UnaryLocalPoint::computeError()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (local_pt(i) < objectscale(i))
            ...
            point_edge_error(i) =
                max_outside_margin_ratio *
                objectscale(i); // if points two far, give a constant error, don't optimize.
    }
