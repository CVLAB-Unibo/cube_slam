...
System::System(const string &strVocFile, const eSensor sensor, bool use_loop_closing)
    : mSensor(sensor), use_loop_close(use_loop_closing), mbReset(false),
      mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    mpTracker =
        new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer, mpMap, mpKeyFrameDatabase,
                     mSensor); // setting file read by tracker, not by mapper!
...
void System::Shutdown()
{
    ...
    while (!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || !mpViewer->isFinished() ||
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    ofstream f(filename, std::ofstream::out | std::ofstream::trunc);
    f << fixed;
...
void System::SaveTrajectoriesKITTI(const string &filename)
{
    cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
    ...
    if (mSensor == MONOCULAR)
    {
        ...
        std::cout << "reference first and ground init first   " << first_reference_id << "   "
                  << absolute_scale_frame_id << std::endl;
    }

    f.close();
    cout << endl << "trajectory saved!" << endl;

    // mpTracker->SaveOptimizedCuboidsToTxt();
}
