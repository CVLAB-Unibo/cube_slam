...
public:
    ...
    // Associate a "right" coordinate to a keypoint if there is valid depth in the depthmap. update
    // mvuRight, mvDepth
...
public:
    ...
    // Threshold close/far points. Close points are inserted from 1 view.   = ThDepth (yaml file)  *
    // actual_baseline Far points are inserted as in the monocular case from 2 views.
    float mThDepth;
    ...
    // system). all keypoints are in same raw image space. keypoints have octaveID. In the stereo
    // case, mvKeysUn is redundant as images must be rectified. In the RGB-D case, RGB images can be
    // distorted.
    ...
    static long unsigned int nNextId; // NOTE this is global static, will automatically increase
                                      // when keyframe is created.
    long unsigned int mnId;

    // Reference Keyframe.  the past keyframe which shares most MapPoints with this frame. if
    // current frame later becomes keyframe. then it is itself
