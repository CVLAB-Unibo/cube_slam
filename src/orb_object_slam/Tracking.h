...
public:
    ...
    std::vector<Eigen::MatrixXd> all_offline_object_cubes; // each n*12 read all txt together so
                                                           // that don't need to read on the fly
...
protected:
    ...
    void CreateInitialMapMonocular(); // create map keyframes (the two frames which can initialize)
                                      // and keypoints

    void CheckReplacedInLastFrame();
    bool TrackReferenceKeyFrame(); // frame to keyframe tracking. use ORB feature descriptors for
                                   // matching (in 2D), instead of projection.
    ...
    bool TrackLocalMap();     // frame to local map, more points involved compared to
                              // TrackWithMotionModel(). only one frame. map fixed.
    ...
    std::vector<KeyFrame *>
        mvpLocalKeyFrames; // local visibile or covisible keyframes../ not include just created KF
