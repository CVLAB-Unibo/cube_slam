...
{
    mKeyFrameSize = ORB_SLAM2::otherSettings["Viewer.KeyFrameSize"];
    mKeyFrameLineWidth = ORB_SLAM2::otherSettings["Viewer.KeyFrameLineWidth"];
    mGraphLineWidth = ORB_SLAM2::otherSettings["Viewer.GraphLineWidth"];
    mPointSize = ORB_SLAM2::otherSettings["Viewer.PointSize"];
    mCameraSize = ORB_SLAM2::otherSettings["Viewer.CameraSize"];
    mCameraLineWidth = ORB_SLAM2::otherSettings["Viewer.CameraLineWidth"];
...
void MapDrawer::DrawMapPoints()
{
    ...
    const vector<MapPoint *> &vpRefMPs =
        mpMap->GetReferenceMapPoints(); // local covisible map points. a subset of vpMPs

    set<MapPoint *> spRefMPs(vpRefMPs.begin(),
                             vpRefMPs.end()); // why use set? any duplication in vpRefMPs
