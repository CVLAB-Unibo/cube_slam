                                   ...
                                   vector<MapPoint *> &vpMatched, int th)
{
    ...
    for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++)
    {
        ...
        int nPredictedLevel =
            pMP->PredictScale(dist, pKF->mfLogScaleFactor); // how to get the level??
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for (int i = 0; i < nMPs; i++)
    {
        ...
        // If there is already a MapPoint replace otherwise add new measurement  if projected pixel
        // location and descriptor are both similar, replace it!
        if (bestDist <= TH_LOW)
        {
